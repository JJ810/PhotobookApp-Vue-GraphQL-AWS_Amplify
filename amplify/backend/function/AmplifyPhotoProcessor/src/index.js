...
// Resize image to thumbnail and convert to JPEG; then put in S3
async function createThumbnail(bucket, key, image) {
    ...
    try {
        ...
        await s3Client.putObject({ Bucket: bucket, Key: thumbKey, Body: thumb })
            .promise()
        return {
            bucket: bucket,
...
async function updatePhotoRecord(photoId, metadata, thumbnail) {
    ...
    if (result.errors && result.errors.length > 0) {
        console.log(`[updatePhotoRecord] ${result.errors[0].message}`)
        throw new Error(
...
exports.handler = async event => {
    ...
    try {
        ...
        let [thumb, metadata] = await Promise.all([createThumbnail(bucket, key, image.Body), getPhotoMetadata(image.Body)])
